                ...
                new RequestCallback<CreateTeamResult>() {
                    ...
                    public void onSuccess(CreateTeamResult result) {
                        ...
                        if (failedAccounts != null && !failedAccounts.isEmpty()) {
                            ...
                            ToastHelper.showToast(Appstorage.getContext(), com.netease.nim.uikit.R.string.create_team_success);
                        }

                        if (isNeedBack) {
                         //   SessionHelper.startTeamSession(context, result.getTeam().getId(), MainActivity.class, null); // 进入创建的群
                            NimUIKit.startChatting(context, result.getTeam().getId(), SessionTypeEnum.Team, null, TeamListActivity.class, null);
                        } else {
                            //SessionHelper.startTeamSession(context, result.getTeam().getId());
                            NimUIKit.startTeamSession(context, result.getTeam().getId());
                        }
                        if (callback != null) {
                    ...
                    public void onFailed(int code) {
                        ...
                        if (code == ResponseCode.RES_TEAM_ECOUNT_LIMIT) {
                            ...
                            ToastHelper.showToast(Appstorage.getContext(), tip);
                        } else {
                            ToastHelper.showToast(Appstorage.getContext(), com.netease.nim.uikit.R.string.create_team_failed);
                        }
