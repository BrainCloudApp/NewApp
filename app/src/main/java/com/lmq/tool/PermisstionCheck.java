...
public class PermisstionCheck {
    ...
    @RequiresApi(api = Build.VERSION_CODES.M)
    public static boolean checkAndRequestPermission(Activity activity,String action) {
        ...
        try {
            ...
            if (!(activity.checkSelfPermission(action) == PackageManager.PERMISSION_GRANTED)) {
                lackedPermission.add(action);
            }
